...
//			+ "	     ,  isSabina\r\n"
	...
	private String declareTempBatchRPAP = "\r\n"
			...
			+ "      ,[PO]\r\n"
			+ "      ,[POLine]\r\n"
			+ "      , CAST(  ROW_NUMBER() OVER ( PARTITION BY a.POId ORDER BY [PO] ,[POLine],[ProductionOrder] )as NVARCHAR(max)) +'/'+ CAST([Batch]as NVARCHAR(max) ) as [Batch]\r\n"
			+ "      ,[ProductionOrder]\r\n"
//			+ "      ,[ApproveStatus] \r\n"
			+ "      ,[Volume]\r\n"
			+ "  into #tempBatchRPAP\r\n"
...
//			+ " declare @StockDateFirst NVARCHAR(2) = '26'; \r\n"
//			+ " declare @StockDateLast NVARCHAR(2) = '25' ; \r\n" 
			+ " declare @StockDateFirst NVARCHAR(2) = CAST ( "
			+ "                                      ( SELECT [DayOfMonth] + 1 FROM [PPMM].[dbo].[StockDefaultDate] where [DataStatus] ='O' )"
			+ "                                       AS NVARCHAR(2)"
			+ "                                      ); \r\n"
			+ " declare @StockDateLast NVARCHAR(2) = CAST ( "
			+ "										( SELECT [DayOfMonth] FROM [PPMM].[dbo].[StockDefaultDate] where [DataStatus] ='O' )"
			+ "										 AS NVARCHAR(2)"
			+ "										) ; \r\n" 
			 
			+ " declare @FirstYear NVARCHAR(4)  = year( dateadd(YEAR,-1,@BeginDateFirst))   ;\r\n"
			+ " declare @LastYear NVARCHAR(4)  = year( dateadd(YEAR,1,@BeginDateFirst))  ; \r\n"
			+ " declare @BeginFirstMonth date  = datefromparts(\r\n"
			+ "			CAST( year(@BeginDateZero) as NVARCHAR(4))\r\n"
			+ "			,RIGHT('00' + cast(month(@BeginDateZero) as NVARCHAR(2)),2)\r\n"
			+ "		 ,@StockDateFirst\r\n"
			+ "		 ) ; \r\n"
			+ " declare @LastFirstMonth date  = datefromparts(\r\n"
			+ "			CAST( year(@BeginDateFirst) as NVARCHAR(4))\r\n"
			+ "			,RIGHT('00' + cast(month(@BeginDateFirst) as NVARCHAR(2)),2)\r\n"
			+ "		 ,@StockDateLast\r\n"
			+ "		 )  ; \r\n"
			...
			+ "			CAST( year(@BeginDateSecond) as NVARCHAR(4))\r\n"
			+ "			,RIGHT('00' + cast(month(@BeginDateSecond) as NVARCHAR(2)),2)\r\n"
			+ "		 ,@StockDateLast\r\n"
			+ "		 )  ;\r\n"
			...
			+ "			CAST( year(@BeginDateThird) as NVARCHAR(4))\r\n"
			+ "			,RIGHT('00' + cast(month(@BeginDateThird) as NVARCHAR(2)),2)\r\n"
			+ "		 ,@StockDateLast\r\n"
			+ "		 )  ;\r\n"
...
//			+ this.leftJoinPOStatusRDDCD
	...
	private String declareFCTempProd = ""
			...
			+ "	   , CAST(  ROW_NUMBER() OVER ( PARTITION BY a.[ForecastId]  ORDER BY a.[ForecastId] , a.[ProductionOrder] )as NVARCHAR(max))  as [BatchFirst] \r\n"
			+ "  into #tempFCTempProd\r\n"
			+ "  FROM [PPMM].[dbo].[SOR_TempProd] AS A\r\n"
...
//				+ " where ReDyeStatus = 'O' "+wherePO +" \r\n"
	...
	public ArrayList<SummaryMonthlyCapReportDetail> getSummaryMonthlyCapReportDetail(
		...
		String sql = ""
				...
				+ "     1 as No\r\n"
				+ "     ,(\r\n"
				+ "			 RIGHT('00' + cast(month(@LastFirstMonth) as NVARCHAR(2)),2)+'/'+\r\n"
				+ "			CAST( year(@LastFirstMonth) as NVARCHAR(4))\r\n"
				+ "		) as DateBeginLast\r\n"
				+ "		, ( \r\n"
				...
				+ "     2 as No\r\n"
				+ "     ,(\r\n"
				+ "			 RIGHT('00' + cast(month(@LastSecondMonth) as NVARCHAR(2)),2)+'/'+\r\n"
				+ "			CAST( year(@LastSecondMonth) as NVARCHAR(4))\r\n"
				+ "		) as DateBeginLast\r\n"
				+ "		, ( \r\n"
				+ "			select   COUNT(WorkDate)   \r\n"
				+ "			from #tempWorkDate as a\r\n"
				...
				+ "     3 as No\r\n"
				+ "     ,(\r\n"
				+ "			 RIGHT('00' + cast(month(@LastThirdMonth) as NVARCHAR(2)),2)+'/'+\r\n"
				+ "			CAST( year(@LastThirdMonth) as NVARCHAR(4))\r\n"
				+ "		) as DateBeginLast\r\n"
				+ "		, ( \r\n"
				+ "			select   COUNT(WorkDate)   \r\n"
				+ "			from #tempWorkDate as a\r\n"
...
//
	...
	public ArrayList<TotalGroupDyeDetail> getTotalGroupDyeDetail(ArrayList<InputDateRunningDetail> poList) {
		...
		String sql = ""
				+ declarePlan
				+ this.declareMonthly
				+ "  SELECT c.[WorkDate] as PlanDate, b.[GroupNo],b.[SubGroup]  ,C.[NormalWork] AS IsHolilyDay,CAST(d.[TotalLot] AS NVARCHAR) as  TotalLot\r\n"
				+ " FROM [PPMM].[dbo].[InputMainGroupDetail] AS A\r\n"
				+ " INNER JOIN [PPMM].[dbo].[InputSubGroupDeail] AS B ON A.[GroupNo] = B.[GroupNo]    \r\n"
