...
// 				} ,     
				...
				{ targets : [ 40 ],     
			   	  render: function (data, type, row) {	     
			   		var htmlEx = '';
			   		if(row.TypePrd == "Replaced"|| row.TypePrd == "Switch"|| row.TypePrd == "OrderPuang"||row.LotNo == "รอจัด Lot"	 || row.LotNo == "ขาย stock" ){ 
						htmlEx = ''; 
					}
					else{   
				...
				{ targets : [ 41 ],     
			   	  	render: function (data, type, row) {	     
					var htmlEx = ''          
// 					console.log( row.TypePrd,row.TypePrdRemark )
					if( row.TypePrd == "Switch" && row.TypePrdRemark == "MAIN" ){ 
// 						htmlEx = '<button type="button" class="btn btn-warning btn-cancelSW">ยกเลิกโยกขาย</button>'; 
					...
					else if(row.TypePrd == "Replaced" || row.TypePrd == "OrderPuang"||row.LotNo == "รอจัด Lot"	 || row.LotNo == "ขาย stock" || row.TypePrdRemark == "SUB" || row.TypePrdRemark == ""){ 
						htmlEx = ''; 
					}     
					else{   
						htmlEx = '<input class="form-control SwitchRemarkInput" style=" cursor: pointer; padding: 4px 2px;font-size: 12.5px"maxlength="200"  name="StockRemark" type="text"  value = "' + row.SwitchRemark+ '" autocomplete="off" >'; 
...
// 			checkReplaced= 0;   
  ...
  $("#SWMainTable").on("keydown blur",".SwitchRemarkInput", function (e) { 
		var $row = $(this).parents("tr");
		var idx = SWMainTable.row($row).index(); 
//		console.log(idx)
		var rowData = SWMainTable.row($row).data(); 
		var oldValue = rowData.SwitchRemark.trim();  
		var prodOrder = rowData.ProductionOrder.trim();  
       var newValue = $(this).val().trim();      
		 if (event.keyCode === 13) {         
			 ...
			 e.preventDefault();      
			 if(prodOrder != newValue){
				handlerInputField("SwitchRemark" ,oldValue,newValue,check1,rowData ,SWMainTable,idx)
			 }
			 else{
				 swal({   
			   		    title: 'Warning',      
  ...
  $("#MainTable").on("keydown blur",".PCRemarkInput", function (e) { 
        ...
        var newValue = $(this).val();      
		 if (event.keyCode === 13) {         
			 e.stopImmediatePropagation();
			 e.preventDefault();  
			 handlerInputField("PCRemark" ,oldValue,newValue,check1,rowData ,MainTable,idx)
// 			 checkReplaced = 1 ;      
// 			swal({ 
		 ...
		 if (event.keyCode === 13) {         	
			 ...
			 let mainSplit = newValue.split(",");
				...
				if(!checkDigit)  {
					...
					handlerInputField("ReplacedRemark" ,oldValue,newValue,check1,rowData ,MainTable,idx)   
				}  
		 }
// 			swal({ 
        ...
        var newValue = $(this).val();      
		 if (event.keyCode === 13) {         
			 ...
			 if(rowData.Grade == ''){
				 ...
				 handlerInputField("StockRemark" ,oldValue,newValue,check1,rowData ,MainTable,idx)
// 				 checkReplaced = 1 ;      
// 				 swal({ 
...
function getSwitchProdOrderDetailByPrd(fieldName,arrayTmp) {  
	var prodOrderSW= arrayTmp[0].SwitchRemark;
	var prodOrder= arrayTmp[0].ProductionOrder;
	if(prodOrderSW == ''){
	...
	$.ajax({   
		...
		success: function(data) {   
			if(data.length > 0){  
				var bean = data[0];  
				if(bean.IconStatus == 'I'){
// 					getSwitchProdOrderDetailByPrd(arrayTmp,data) ;
					swal({
						title: "Success",    
					 	text: bean.SystemStatus ,
						icon: "info",
