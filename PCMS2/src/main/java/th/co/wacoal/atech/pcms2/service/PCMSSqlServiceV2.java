...
public class PCMSSqlServiceV2 {
	...
	public String createTempFlagHasRP = ""
			+ PCMSSqlServiceV2.buildIfTempTableDrop("#FlagHasRP")
			+ "SELECT DISTINCT SaleOrder, SaleLine\r\n"
			+ "INTO #FlagHasRP\r\n"
			+ "FROM PCMS.dbo.ReplacedProdOrder\r\n"
			+ "WHERE DataStatus = 'O';\r\n"
//			+ "CREATE CLUSTERED INDEX IX_FlagHasRP ON #FlagHasRP(SaleOrder, SaleLine);\n"
	;
	public String createTempFlagHasOP = ""
			+ PCMSSqlServiceV2.buildIfTempTableDrop("#FlagHasOP")
			+ "SELECT DISTINCT SaleOrder, SaleLine\r\n"
			+ "INTO #FlagHasOP\r\n"
			+ "FROM PCMS.dbo.FromSapMainProdSale\r\n"
			+ "WHERE DataStatus = 'O';\r\n"
//			+ "CREATE CLUSTERED INDEX IX_FlagHasOP ON #FlagHasOP(SaleOrder, SaleLine);;\n"
	;
	public String createTempProdAgg = ""
			+ PCMSSqlServiceV2.buildIfTempTableDrop("#tmpProdAgg")
			+ "SELECT \r\n"
			+ "    p.[ProductionOrder],\r\n"
...
//			+ "ON #tmpProdAgg(ProductionOrder);\n"
	...
	public String createTempSaleAgg = ""
			+ PCMSSqlServiceV2.buildIfTempTableDrop("#tmpSaleAgg")
			+ "SELECT \r\n"
			+ "    SaleOrder,\r\n"
			+ "    SaleLine,\r\n"
			+ "    SUM(COALESCE(t.SumVolOP,0)) AS SumVolOP,\r\n"
...
//			+ "ON #tmpSaleAgg(SaleOrder, SaleLine);\n"
	...
	public String createTempSumVolRP = ""
			+ PCMSSqlServiceV2.buildIfTempTableDrop("#tmpSumVolRP")
			+ "SELECT \r\n"
			+ "   a.ProductionOrderRP,\r\n"
...
//			+ "CREATE CLUSTERED INDEX IX_tmpSumVolRP ON #tmpSumVolRP(ProductionOrderRP);\n"
	...
	public String createTempBillBatchFlag = ""
			+ PCMSSqlServiceV2.buildIfTempTableDrop("#BillBatchFlag")
...
//			+ "CREATE CLUSTERED INDEX IX_BBF ON #BillBatchFlag(SaleOrder, SaleLine);\n"
	...
	public String createTempSumVolOP = ""
			+ PCMSSqlServiceV2.buildIfTempTableDrop("#tmpSumVolOP")
			+ "SELECT \r\n"
			+ "   a.ProductionOrder,\r\n"
...
//			+ "CREATE CLUSTERED INDEX IX_tmpSumVolOP ON #tmpSumVolOP(ProductionOrder);\n"
	...
	public String createTempSumGR = ""
			+ PCMSSqlServiceV2.buildIfTempTableDrop("#tempSumGR")
			+ " SELECT *\r\n"
			+ " into #tempSumGR\r\n"
			+ " FROM [PCMS].[dbo].SumGRCache  ;\r\n";
	public String createTempSumBill = ""
			+ PCMSSqlServiceV2.buildIfTempTableDrop("#tempSumBill")
			+ " SELECT *\r\n"
			+ " into #tempSumBill\r\n"
			+ " FROM [PCMS].dbo.SumBillCache ; \r\n";
	public String createTempMainSale = ""
			+ PCMSSqlServiceV2.buildIfTempTableDrop("#tempMainSale")
			+ " SELECT   \r\n"
			+ "	   a.*\r\n"
			+ " INTO #tempMainSale \r\n"
	...
	public String createTempPlanDeliveryDate = "  "
			+ PCMSSqlServiceV2.buildIfTempTableDrop("#tempPlandeliveryDate")
			+ " ;WITH LatestPlan AS (\r\n"
			+ "    SELECT\r\n"
			+ "        Id,\r\n"
			+ "        [ProductionOrder],\r\n"
...
//			+ "CREATE CLUSTERED INDEX IX_tempPlanDelivery ON #tempPlandeliveryDate (ProductionOrder, SaleOrder, SaleLine); \n"
	...
	public String createTempCRP = ""
			+ PCMSSqlServiceV2.buildIfTempTableDrop("#tmpCRP")
	...
	public String leftJoinBPartOneT_A = "" + " LEFT JOIN #tmpSumVolOP t ON b.ProductionOrder = t.ProductionOrder\r\n";
	public String leftJoinBPartOneS_A = "" + " LEFT JOIN #tmpSumVolRP s ON b.ProductionOrder = s.ProductionOrderRP\r\n";
...
//			"" + "CREATE CLUSTERED INDEX IX_tempMainSale_SO_SL\r\n" + "ON #tempMainSale (SaleOrder, SaleLine);";
	...
	public String createTempPrepWaitLot = ""
			+ PCMSSqlServiceV2.buildIfTempTableDrop("#tempPrepWaitLot")
			+ " SELECT \r\n"
...
//		whereClauses.put("whereBMainUserStatus", whereBMainUserStatus);
	...
	public String getLeftJoinTAPP(String aliasProd)
	{
		String sqlTemplate = " left join ( \r\n"
				...
				+ "    inner join [PPMM].[dbo].[ApprovedPlanDate] as b on a.POId = b.POId and b.DataStatus = 'O'  \r\n"
				+ "    WHERE a.DataStatus = 'O' \r\n"
...
//				+ "        AND CRP.SaleLine = %s.SaleLine \r\n";
	...
	public String getLeftJoinFromSapMainProd(String aliasSale)
	{
		String sqlTemplate =
				"" + " LEFT JOIN PCMS.dbo.FromSapMainProd fsmp ON fsmp.ProductionOrder = %s.ProductionOrder \r\n" + "   ";
		return String.format(sqlTemplate, aliasSale);
	}
...
//	}
	...
	public static String buildIfTempTableDrop(String tempName)
	{
		String sqlTemplate =
	...
	private static final String FINAL_TEMPLATE =
			"IF OBJECT_ID('tempdb..#tempAllProdBase') IS NOT NULL DROP TABLE #tempAllProdBase;\n"
					+ "SELECT * INTO #tempAllProdBase\n"
					+ "FROM (\n"
					+ "  %s\n"
					+ // <<-- UNION ทั้งหมดมายัดตรงนี้
					") x;\n";
//	private static final String CASE_MAIN =
//		    "SELECT  FSMS.SaleOrder, FSMS.SaleLine, FSMP.ProductionOrder, FSMP.LotNo AS LotNoValue,\n" +
//		    "        (FSMP.Volumn - COALESCE(c.SumVolOP,0) - COALESCE(c.SumVolRP,0)) AS Volumn,\n" +
//		    "        UCALRP.UserStatusCal AS UserStatus, m.Grade, m.GRSumMR, m.GRSumKG, m.GRSumYD,\n" +
//		    "        'Main' AS TypePrd, 'Main' AS TypePrdRemark\n" +
//		    "FROM #tempMainSale FSMS\n" +
//		    "LEFT JOIN PCMS.dbo.FromSapMainProd FSMP\n" +
//		    "  ON FSMP.SaleOrder = FSMS.SaleOrder AND FSMP.SaleLine = FSMS.SaleLine AND FSMP.DataStatus = 'O'\n" +
//		    "LEFT JOIN #tmpProdAgg c ON c.ProductionOrder = FSMP.ProductionOrder\n" +
//		    "LEFT JOIN #tempSumGR m ON FSMP.ProductionOrder = m.ProductionOrder\n" +
//		    "LEFT JOIN [PCMS].[dbo].[TEMP_UserStatusAuto] UCALRP\n" +
//		    "  ON UCALRP.DataStatus = 'O' AND FSMP.ProductionOrder = UCALRP.ProductionOrder\n" +
//		    "WHERE FSMS.DataStatus = 'O'\n" +
//		    "  AND FSMP.ProductionOrder IS NOT NULL\n" +
//		    "  AND NOT EXISTS (\n" +
//		    "      SELECT 1 FROM PCMS.dbo.SwitchProdOrder SPO\n" +
//		    "      WHERE SPO.ProductionOrderSW = FSMP.ProductionOrder AND SPO.DataStatus = 'O'\n" +
//		    "  )\n" +
//		    "  %s\n";   // <<-- filter slot (Lot/UserStatus)
	private static final String CASE_MAIN =
			"SELECT  FSMS.SaleOrder, FSMS.SaleLine, FSMP.ProductionOrder, FSMP.LotNo AS LotNoValue,\n"
					+ "        (FSMP.Volumn - COALESCE(c.SumVolOP,0) - COALESCE(c.SumVolRP,0)) AS Volumn,\n"
					+ "        UCALRP.UserStatusCal AS UserStatus, m.Grade, m.GRSumMR, m.GRSumKG, m.GRSumYD,\n"
					...
					+ "\r\n"
					+ "LEFT JOIN [PCMS].[dbo].[TEMP_UserStatusAuto] UCALRP\r\n"
					+ "  ON UCALRP.DataStatus = 'O' AND FSMP.ProductionOrder = UCALRP.ProductionOrder  and UCALRP.Grade = m.Grade\n"
					+ "WHERE FSMS.DataStatus = 'O'\n"
					+ "  AND FSMP.ProductionOrder IS NOT NULL\n"
					+ "  AND NOT EXISTS (\n"
					+ "      SELECT 1 FROM PCMS.dbo.SwitchProdOrder SPO\n"
					...
					+ "  )\n"
					+ "  %s\n"; // <<-- filter slot (Lot/UserStatus)
	private static final String CASE_WAITLOT = "SELECT  FSMS.SaleOrder, FSMS.SaleLine, N'รอจัด Lot' AS ProductionOrder,\n"
			+ "        ca.LotNoValue, 0 AS Volumn, '' AS UserStatus, '' AS Grade,\n"
			+ "        0 AS GRSumMR, 0 AS GRSumKG, 0 AS GRSumYD,\n"
			+ "        'WaitLot' AS TypePrd, 'SUB' AS TypePrdRemark\n"
			+ "FROM #tempMainSale FSMS\n"
			+ "LEFT JOIN #FlagHasOP fop ON fop.SaleOrder = FSMS.SaleOrder AND fop.SaleLine = FSMS.SaleLine\n"
			+ "LEFT JOIN #FlagHasRP frp ON frp.SaleOrder = FSMS.SaleOrder AND frp.SaleLine = FSMS.SaleLine\n"
			+ "LEFT JOIN #BillBatchFlag bbf on bbf.SaleOrder = fsms.SaleOrder and bbf.SaleLine = fsms.SaleLine\n"
			+ "CROSS APPLY (\n"
			+ "  SELECT CASE\r\n"
			+ "    WHEN bbf.SaleOrder IS NOT NULL THEN N'Lot ขายแล้ว'\n"
			+ "    WHEN FSMS.SaleStatus = 'C' THEN N'ขาย stock'\r\n"
			+ "    WHEN FSMS.MaterialNo LIKE 'V%' THEN N'รับจ้างถัก'\n"
			+ "    WHEN fop.SaleOrder IS NOT NULL THEN N'พ่วงแล้วรอสวม'\n"
			+ "    WHEN frp.SaleOrder IS NOT NULL THEN N'รอสวมเคยมี Lot'\n"
			+ "    ELSE N'รอจัด Lot'\n"
			+ "  END AS LotNoValue\n"
			+ ") ca\n"
			+ "JOIN #tempLotNoList l ON l.LotNo = ca.LotNoValue\n"
			+ // 🔥 เฉพาะ WaitLot
			"WHERE FSMS.DataStatus = 'O'\n"
			+ "  AND NOT EXISTS (\n"
			+ "      SELECT 1 FROM PCMS.dbo.FromSapMainProd FSMP2\n"
			+ "      WHERE FSMP2.SaleOrder = FSMS.SaleOrder\n"
			+ "        AND FSMP2.SaleLine  = FSMS.SaleLine\n"
			+ "        AND FSMP2.DataStatus = 'O'\r\n"
			+ "			  AND NOT EXISTS (\r\n"
			+ "					SELECT 1\r\n"
			+ "					FROM PCMS.dbo.SwitchProdOrder SPO\r\n"
			+ "					JOIN PCMS.dbo.FromSapMainProd FSMP\r\n"
			+ "						 ON FSMP.ProductionOrder = SPO.ProductionOrderSW \r\n"
			+ "						AND FSMP.DataStatus = 'O'\r\n"
			+ " 					INNER JOIN [PCMS].[dbo].[viewUserStatusMappingPCMS] AS viewUSM_SPE \r\n"
			+ "						ON FSMP.UserStatus = viewUSM_SPE.UserStatus \r\n"
			+ "					 and viewUSM_SPE.[Special] = 1   \r\n"
			+ "					WHERE SPO.ProductionOrderSW = FSMP2.ProductionOrder\r\n"
			+ "					  AND SPO.DataStatus = 'O'\r\n"
			+ "			  )\n"
			+ "  )\n";
	private static final String CASE_ORDER_PUANG = ""
			+ " SELECT FSMPS.SaleOrder, FSMPS.SaleLine, FSMPS.ProductionOrder, FSMP.LotNo,\n"
			+ "       FSMPS.Volumn, UCALRP.UserStatusCal AS UserStatus,\n"
			+ "       m.Grade, m.GRSumMR, m.GRSumKG, m.GRSumYD,\n"
			+ "       'OrderPuang' AS TypePrd, 'SUB' AS TypePrdRemark\n"
			+ "FROM PCMS.dbo.FromSapMainProdSale FSMPS\n"
			+ "JOIN #tempMainSale FSMS\n"
			+ "  ON FSMS.SaleOrder = FSMPS.SaleOrder AND FSMS.SaleLine = FSMPS.SaleLine AND FSMS.DataStatus = 'O'\n"
			+ "JOIN PCMS.dbo.FromSapMainProd FSMP\n"
			+ "  ON FSMP.ProductionOrder = FSMPS.ProductionOrder AND FSMP.DataStatus = 'O'\n"
			+ "LEFT JOIN #tempSumGR m ON FSMPS.ProductionOrder = m.ProductionOrder\n"
			+ "LEFT JOIN [PCMS].[dbo].[TEMP_UserStatusAuto] UCALRP\n"
			+ "  ON UCALRP.DataStatus = 'O' AND FSMPS.ProductionOrder = UCALRP.ProductionOrder\r\n"
			+ "  AND (m.Grade = UCALRP.Grade OR m.Grade IS NULL) \n"
			+ "WHERE FSMPS.DataStatus = 'O'\n"
			+ "  %s\n"; // <<-- filter slot

	private static final String CASE_REPLACED = "    SELECT  \r\n"
			+ "          RP.SaleOrder\r\n"
			+ "        , RP.SaleLine\r\n"
			+ "        , RP.ProductionOrderRP\r\n"
			+ "        , FSMP.[LotNo] \r\n"
			+ "        , CASE WHEN ISNULL(RP.Volume,0) <> 0 THEN RP.Volume\r\n"
			+ "			ELSE FSMP.Volumn\r\n"
			+ "			END AS Volumn\r\n"
			...
			+ "        , 'Replaced'\r\n"
			+ "		, 'SUB'\r\n"
			+ "    FROM PCMS.dbo.ReplacedProdOrder RP\r\n"
			+ "    JOIN PCMS.dbo.FromSapMainProd FSMP \r\n"
			+ "         ON FSMP.ProductionOrder = RP.ProductionOrderRP\r\n"
			+ "        AND FSMP.DataStatus = 'O'\r\n"
			+ " 	INNER JOIN [PCMS].[dbo].[viewUserStatusMappingPCMS] AS viewUSM_SPE \r\n"
			+ "		ON FSMP.UserStatus = viewUSM_SPE.UserStatus \r\n"
			+ "     and viewUSM_SPE.[Special] = 1  \r\n"
			+ "	LEFT JOIN #tempSumGR AS m ON RP.ProductionOrderRP = m.ProductionOrder \r\n"
			+ "	LEFT JOIN [PCMS].[dbo].[TEMP_UserStatusAuto] AS UCALRP \r\n"
			+ "		ON UCALRP.[DataStatus] = 'O' \r\n"
			+ "		 AND RP.ProductionOrderRP = UCALRP.ProductionOrder \r\n"
			+ "		 AND (m.Grade = UCALRP.Grade OR m.Grade IS NULL) \r\n"
			+ "    WHERE RP.DataStatus = 'O'\n"
			+ "  %s\n"; // <<-- filter slot
	private static final String CASE_SWITCH = " SELECT \r\n"
			+ "          SPO.SaleOrderSW\r\n"
			+ "        , SPO.SaleLineSW\r\n"
			+ "        , SPO.ProductionOrderSW\r\n"
			+ "        , FSMP.[LotNo] \r\n"
			+ "        , FSMP.Volumn\r\n"
			+ "		, UCALRP.UserStatusCal AS UserStatus\r\n"
			+ "		, m.Grade\r\n"
			+ "        , m.GRSumMR\r\n"
			+ "        , m.GRSumKG\r\n"
			+ "        , m.GRSumYD\r\n"
			+ "        , 'Switch'\r\n"
			+ "		, CASE\r\n"
			+ "			when SPO.ProductionOrder = SPO.ProductionOrderSW then 'MAIN' ELSE 'SUB' END TypePrdRemark\r\n"
			+ "    FROM PCMS.dbo.SwitchProdOrder SPO\r\n"
			+ "    JOIN PCMS.dbo.FromSapMainProd FSMP\r\n"
			+ "         ON FSMP.ProductionOrder = SPO.ProductionOrderSW \r\n"
			+ "        AND FSMP.DataStatus = 'O'\r\n"
			...
			+ "		ON FSMP.UserStatus = viewUSM_SPE.UserStatus \r\n"
			+ "     and viewUSM_SPE.[Special] = 1    \r\n"
			+ "	LEFT JOIN #tempSumGR AS m ON SPO.ProductionOrderSW = m.ProductionOrder \r\n"
			+ "	LEFT JOIN [PCMS].[dbo].[TEMP_UserStatusAuto] AS UCALRP \r\n"
			+ "		ON UCALRP.[DataStatus] = 'O' \r\n"
			+ "		 AND SPO.ProductionOrderSW = UCALRP.ProductionOrder \r\n"
			+ "		 AND (m.Grade = UCALRP.Grade OR m.Grade IS NULL) \r\n"
			+ "	WHERE SPO.DataStatus = 'O'\n"
			+ "  %s\n"; // <<-- filter slot
	private static final String COMMON_FILTER =
			"AND (\n" + " EXISTS (SELECT 1 FROM #tempUserStatusList u WHERE u.UserStatus = %s)\n" + ")\n";

	public String finalIntoTempAllProdBase(String whereProdFinal)
	{

		String filterLotUser = String.format(COMMON_FILTER, "UCALRP.UserStatusCal");
		String filterLotUserReplaced = String.format(COMMON_FILTER, "UCALRP.UserStatusCalRP");
		String waitLotSql = "";
		String puangSql = "";
		String replaceSql = "";
		String switchSql = "";
		String mainSql = String.format(CASE_MAIN, whereProdFinal+filterLotUser);
		if ( ! whereProdFinal.equals("")) {
			waitLotSql = ""
					+ " SELECT TOP 0 null as SaleOrder, null as SaleLine, null as ProductionOrder, null as LotNoValue,\r\n"
					+ "        null as Volumn,\r\n"
					+ "        null as UserStatus, null as Grade, null as GRSumMR,null as GRSumKG, null as GRSumYD,\r\n"
					+ "        null as TypePrd, null as TypePrdRemark \n"
					+ " FROM #tempMainSale"; // คืนแค่โครงสร้างแต่ไม่มี Data และไม่เสียเวลาคำนวณ JOIN เยอะๆ

			puangSql = String.format(CASE_ORDER_PUANG,
					whereProdFinal.replace("fsmp.ProductionOrder", "FSMPS.ProductionOrder")+filterLotUser);
			switchSql = String.format(CASE_SWITCH,
					whereProdFinal.replace("fsmp.ProductionOrder", "SPO.ProductionOrderSW")+filterLotUser);
			replaceSql = String.format(CASE_REPLACED,
					whereProdFinal.replace("fsmp.ProductionOrder", "rp.ProductionOrderRP")+filterLotUser);
		} else {
			// กรณีปกติ (whereProdFinal เป็นค่าว่าง)
			waitLotSql = CASE_WAITLOT;
			puangSql = String.format(CASE_ORDER_PUANG, filterLotUser);
			switchSql = String.format(CASE_SWITCH, filterLotUser);
			replaceSql = String.format(CASE_REPLACED, filterLotUserReplaced);
		}
		// String replSql = ...
		// String swSql = ...

		String unionAll = String.join("\nUNION ALL\n", mainSql, waitLotSql, puangSql, replaceSql, switchSql
		// , replSql, swSql
		);

		return String.format(FINAL_TEMPLATE, unionAll);
	}

	public static String MAIN_SALE = ""
			+ buildIfTempTableDrop("#tempMainSale")
			+ " SELECT   \r\n"
			+ "	   a.*\r\n"
			+ " INTO #tempMainSale \r\n"
			+ " FROM [PCMS].[dbo].[FromSapMainSale] as a\r\n"
	...
	public String finalIntoTempMainSale(String whereSale)
	{
		return String.format(MAIN_SALE, whereSale);
	}
}